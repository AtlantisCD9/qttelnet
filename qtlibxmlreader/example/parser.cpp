...
{
    m_result += nestPrefix();
    m_result += "internalEntityDecl(name=\"" + escapeStr(name)
		    + "\", value=\"" + escapeStr(value) + "\")\n";
    return TRUE;
}
...
Parser::Parser(QXmlReader *reader)
{
    m_reader = reader;

    m_handler = new ContentHandler;
...
bool Parser::parse(QIODevice *src)
{
    QXmlInputSource source(src);
    return m_reader->parse(source);
}

bool Parser::parse(const QString &file_name)
{
    return static_cast<QtLibxmlReader*>(m_reader)->parse(file_name);
}
